...
module.exports = env => {
  ...
  return {
    ...
    // Suppress the known pre-existing "Critical dependency: dynamic expression in require()"
    // warning that comes from the local-plugin hot-loader in useInitPlugins.ts.
    // That dynamic import() is intentional (loads ./plugins/** at dev time) and is
    // documented with a comment in the source. It does NOT affect runtime behaviour.
    ignoreWarnings: [
      {
        module: /useInitPlugins\.ts/,
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        ...
        DEMO_MODE: JSON.stringify(!!env.demo),
      }),
